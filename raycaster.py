...
class RayCaster:
    ...
    def calculate_ray(self, map_data: List[List[str]], player_pos: Tuple[int, int], 
                     player_angle: float, ray_angle: float) -> Tuple[float, str]:
        x, y = player_pos
    ...
    def render_frame(self, map_data: List[List[str]], player_pos: Tuple[int, int], 
                    player_angle: float) -> List[str]:
        frame = []
        ...
        for y in range(self.screen_height):
            ...
            for x in range(self.screen_width):
                ...
                ray_angle = player_angle - self.FOV/2 + (x/self.screen_width) * self.FOV
                
                distance, wall_type = self.calculate_ray(map_data, player_pos, player_angle, ray_angle)
                
                # Calculate wall height based on distance
                wall_height = (self.screen_height / distance) if distance > 0 else self.screen_height
