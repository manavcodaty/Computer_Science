...
class World:
    ...
    def _generate_feature(self, map_grid: List[List[str]], x: int, y: int, 
                         feature: str, spread: float) -> None:
        for dy in range(-1, 2):
            for dx in range(-1, 2):
    ...
    def check_pokemon_encounter(self) -> None:
        if random.random() < 0.2:
            ...
            possible_pokemon = [p for p in self.pokemon_list 
            ...
            if possible_pokemon:
                ...
                if random.random() < pokemon.rarity:
                    ...
                    if choice.lower() == 'y':
                        catch_rate = 0.5 + (0.2 if current_terrain == pokemon.habitat else 0)
                        if random.random() < catch_rate:
                            print(f"{COLORS['green']}Caught {pokemon.name}!{COLORS['reset']}")
                            self.inventory.append(Pokemon(pokemon.name, pokemon.rarity, pokemon.habitat))
                        else:
                            print(f"{COLORS['red']}{pokemon.name} escaped!{COLORS['reset']}")
                    time.sleep(1)
    ...
    def display(self) -> None:
        ...
        print(f"Direction: {['North', 'East', 'South', 'West'][self.player_direction//90]}")
